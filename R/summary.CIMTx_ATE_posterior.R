...
#'
#' @param object a \code{CIMTx_ATE_posterior} object
#' obtained with \code{\link{ce_estimate}} function.
#' @param ... further arguments passed to or from other methods.
#'
#' @return a list with w*(w-1)/2 elements for ATE effect.
...
summary.CIMTx_ATE_posterior <- function(object, ...) {
  ...
  for (i in 1:(n_trt - 1)) {
    for (j in (i + 1):n_trt) {
      assign(paste0("ate", i, j),
             posterior_summary(object[[paste0("ATE_RD", i, j)]],
                               object[[paste0("ATE_RR", i, j)]],
                               object[[paste0("ATE_OR", i, j)]]))
      assign(paste0("ATE", i, j), list(round(eval(
        parse(text = (paste0("ate", i, j)))
      ), digits = 2)))
      assign(paste0("ATE", i, j), stats::setNames(eval(parse(
        text = (paste0("ATE", i, j))
      )), paste0("ATE", i, j)))
      result_summary <-
        c(result_summary, (eval(parse(
          text = (paste0("ATE", i, j))
        ))))
    }
  }
